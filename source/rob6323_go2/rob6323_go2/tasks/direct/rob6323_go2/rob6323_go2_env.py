...
class Rob6323Go2Env(DirectRLEnv):
    ...
    def __init__(self, cfg: Rob6323Go2EnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # ------------------------------------------------
        # Actuator friction (randomized per-episode)
        # ------------------------------------------------
        # Stored per-env, per-joint (we sample one value per env and broadcast to 12 joints)
        self._mu_v = torch.zeros(self.num_envs, 12, device=self.device)  # viscous coeff
        self._F_s  = torch.zeros(self.num_envs, 12, device=self.device)  # stiction coeff
    ...
    def _setup_scene(self):
        ...
        if getattr(self.cfg, "use_height_scanner", False):
    ...
    def _ground_z_max_under_base(self) -> torch.Tensor:
        """Returns (N,) max ground height hit by rays. Uses -inf for missing hits."""
        hits_z = self._height_scanner.data.ray_hits_w[..., 2]  # (N, R)
        neg_inf = torch.full_like(hits_z, -1.0e6)
        hits_z = torch.where(torch.isfinite(hits_z), hits_z, neg_inf)
        return torch.max(hits_z, dim=1).values
    ...
    def _get_observations(self) -> dict:
        ...
        # --- NEW: terrain heights (optional) ---
        if self._height_scanner is not None:
            base_z = self.robot.data.root_pos_w[:, 2].unsqueeze(-1)          # (N,1)
            hits_z = self._height_scanner.data.ray_hits_w[..., 2]            # (N,R)

            # convert to "height below base" (positive if ground is below base)
            hits_z = torch.where(torch.isfinite(hits_z), hits_z, base_z)
            h = base_z - hits_z                                              # (N,R)
            h = torch.clamp(h, -float(self.cfg.height_scan_clip), float(self.cfg.height_scan_clip))

            # downsample to fixed size so observation_space is stable
            R = h.shape[1]
            M = int(self.cfg.height_scanner_num_points)
            idx = torch.linspace(0, R - 1, M, device=self.device).long()
            obs_parts.append(h[:, idx])
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        if self._height_scanner is not None:
            ...
            base_h = base_z - ground_z                 # height above (highest nearby) ground
        else:
            base_h = base_z
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        self._F_s[env_ids_t]  = F_s_E.unsqueeze(1).repeat(1, 12)
