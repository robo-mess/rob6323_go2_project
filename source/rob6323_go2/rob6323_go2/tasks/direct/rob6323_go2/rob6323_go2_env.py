...
class Rob6323Go2Env(DirectRLEnv):
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        # Start near beginning of terrain strip, with random XY jitter
        j = float(self.cfg.spawn_xy_jitter)
        dx = sample_uniform(-j, j, (len(env_ids_t),), self.device)
        dy = sample_uniform(-j, j, (len(env_ids_t),), self.device)

        root[:, 0] = root[:, 0] + float(self.cfg.spawn_base_x) + dx
        root[:, 1] = root[:, 1] + float(self.cfg.spawn_base_y) + dy
        root[:, 2] = root[:, 2] + 0.08

        # Small yaw jitter (still basically forward)
        jy = float(self.cfg.spawn_yaw_jitter)
        dyaw = sample_uniform(-jy, jy, (len(env_ids_t),), self.device)
        zeros = torch.zeros_like(dyaw)
        yaw_quat = math_utils.quat_from_euler_xyz(zeros, zeros, dyaw)

        root[:, 3:7] = math_utils.quat_mul(root[:, 3:7], yaw_quat)
    ...
    def _step_contact_targets(self):
        ...
        for fi in foot_indices:
            sm_i = (
                cdf(torch.remainder(fi, 1.0)) * (1 - cdf(torch.remainder(fi, 1.0) - 0.5))
                + cdf(torch.remainder(fi, 1.0) - 1) * (1 - cdf(torch.remainder(fi, 1.0) - 1.5))
            )
            sm.append(sm_i)

        self.desired_contact_states[:, 0] = sm[0]
        self.desired_contact_states[:, 1] = sm[1]
        self.desired_contact_states[:, 2] = sm[2]
        self.desired_contact_states[:, 3] = sm[3]
