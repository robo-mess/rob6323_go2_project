...
class Rob6323Go2Env(DirectRLEnv):
    ...
    # ---------------------------
    # Reset (puts every bot on its own tile)
    # ---------------------------
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        # unique origin per env (because we created enough tiles)
        root[:, :3] += self._terrain.env_origins[env_ids]
        root[:, 0] += float(self.cfg.spawn_offset_x)
        root[:, 2] += float(self.cfg.spawn_lift_z)
