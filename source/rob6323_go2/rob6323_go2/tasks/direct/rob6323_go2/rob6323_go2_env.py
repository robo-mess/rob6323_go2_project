...
import numpy as np
import torch
from collections.abc import Sequence# Copyright (c) 2022-2025, The Isaac Lab Project Developers
# All rights reserved.
# rob6323_go2_env.py
# SPDX-License-Identifier: BSD-3-Clause
...
class Rob6323Go2Env(DirectRLEnv):
    ...
    def __init__(self, cfg: Rob6323Go2EnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # undesired contacts (thigh/calf/hip)
        undesired = []
    ...
    # ---------------------------
    # Reset (IMPORTANT: offset spawn so they climb the stairs)
    # ---------------------------
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        # put each robot on its OWN tile origin
        default_root_state[:, :3] += self._terrain.env_origins[env_ids]

        # offset so they start before the stairs and walk across it
        default_root_state[:, 0] += float(self.cfg.spawn_offset_x)
        default_root_state[:, 2] += 0.05  # tiny lift to avoid spawn penetration
