...
class Rob6323Go2Env(DirectRLEnv):
    ...
    def __init__(self, cfg: Rob6323Go2EnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # Ensure Raibert always has a valid foot_indices tensor
        self.foot_indices = torch.zeros(self.num_envs, 4, dtype=torch.float, device=self.device, requires_grad=False)

        # height scan obs buffer (size comes from cfg)
        self._height_scan_obs = torch.zeros(self.num_envs, int(self.cfg.height_scan_num_rays), device=self.device)
    ...
    def _apply_action(self) -> None:
        # MOST STABLE: position targets (keeps default GO2 actuators)
    ...
    def _update_height_scan_obs(self):
        ...
        # if a ray missed, treat it as "ground at base height"
        hits_z = torch.where(torch.isfinite(hits_z), hits_z, base_z)

        rel = hits_z - base_z  # negative => ground below base
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # forward progress (prevents "freeze")
        forward_progress = torch.clamp(self.robot.data.root_lin_vel_b[:, 0], min=0.0)
        ...
        # base height relative to local ground (from ray hits)
        hits_w = self._height_scanner.data.ray_hits_w
        hits_z = hits_w[..., 2]
        base_z = self.robot.data.root_pos_w[:, 2].unsqueeze(1)
        hits_z = torch.where(torch.isfinite(hits_z), hits_z, base_z)
        ...
        rewards = {
            ...
            # FIXED BUG: use yaw_rate_error_mapped (NOT yaw_rate_reward_mapped)
            "track_ang_vel_z_exp": yaw_rate_error_mapped * self.cfg.yaw_rate_reward_scale * self.step_dt,
            "forward_progress": forward_progress * 2.0 * self.step_dt,
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        # conservative termination: only very low base, base collision, upside-down
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        # spawn jitter (works even if cfg doesn't define it)
        j = float(getattr(self.cfg, "spawn_xy_jitter", 0.0))
        if j > 0.0:
            dx = sample_uniform(-j, j, (len(env_ids_t),), self.device)
            dy = sample_uniform(-j, j, (len(env_ids_t),), self.device)
            root[:, 0] += dx
            root[:, 1] += dy
            root[:, 2] += 0.08
